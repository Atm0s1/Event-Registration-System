...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:'Inter',sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#0f0c29,#302b63,#24243e)}
        .card{background:rgba(255,255,255,.07);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,.12);border-radius:20px;padding:40px;max-width:540px;width:90%;color:#e0e0e0}
        h1{font-size:24px;margin-bottom:20px;background:linear-gradient(135deg,#667eea,#764ba2);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
        .msg{padding:10px 14px;margin:6px 0;border-radius:10px;font-size:14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06)}
        .ok{border-left:4px solid #00cec9}
        .err{border-left:4px solid #ff7675}
        .next{margin-top:24px;display:flex;gap:10px;flex-wrap:wrap}
        a.btn{padding:10px 22px;border-radius:12px;text-decoration:none;font-weight:600;font-size:14px;color:#fff;background:linear-gradient(135deg,#667eea,#764ba2);transition:transform .2s}
        a.btn:hover{transform:translateY(-2px)}
        .warn{margin-top:20px;padding:12px;border-radius:10px;font-size:13px;background:rgba(253,203,110,.1);border:1px solid rgba(253,203,110,.3);color:#feca57}
    </style>
</head>
<body>
<div class="card">
    <h1>⚡ Event Management — Installer</h1>
    <?php foreach ($msgs as $m): ?>
        <div class="msg <?= $ok ? 'ok' : 'err' ?>"><?= $m ?></div>
    <?php endforeach; ?>

    <?php if ($ok): ?>
        <div class="next">
            <a class="btn" href="admin/login.php">Admin Panel →</a>
            <a class="btn" href="user/login.php">User Portal →</a>
        </div>
        <div class="warn">⚠️ Delete <code>install.php</code> after installation for security.</div>
    <?php else: ?>
        <div class="warn">Fix the error above and refresh this page.</div>
    <?php endif; ?>
</div>
</body>
</html>
